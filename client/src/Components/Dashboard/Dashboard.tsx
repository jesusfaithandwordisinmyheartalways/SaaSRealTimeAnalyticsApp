...
const aggregateEvents = (events: UserEvents[], resolution: "minute" | "hour" ): AggregateEvent[] => {
...
const Dashboard:React.FC = () => {
  ...
  const filteredEvents = userFilter ? events.filter((data) => data.userId === userFilter)  : events;
      ...
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={chartData}>
          ...
          {eventTypes.map((type, index) => (
            <Line key={index} type="monotone" dataKey={type} stroke="#3b82f6" />
          ))}
        </LineChart>
      </ResponsiveContainer>
    </div>
